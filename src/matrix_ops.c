...
/* Log the assignment-mandated generic error message. */
void symnmf_log_error(void) {
    fprintf(stdout, "An Error Has Occurred\n");
}

/* Allocate a zero-initialized matrix while guarding against overflow. */
...
/* Release matrix storage if the pointer is non-NULL. */
...
/* Load a CSV-like dataset from disk into a newly allocated matrix. */
...
/* Grow a double buffer to ensure it can hold the requested elements. */
static int ensure_capacity(double **buffer, size_t *capacity, size_t required) {
    size_t new_capacity;
    double *tmp;
...
/* Initialize an empty dataset builder structure. */
static void dataset_builder_init(dataset_builder_t *builder) {
    builder->values = NULL;
    builder->capacity = 0;
    builder->count = 0;
...
/* Release resources held by the dataset builder. */
static void dataset_builder_free(dataset_builder_t *builder) {
    if (builder == NULL) {
        return;
    }
...
/* Append a parsed row into the dataset builder, tracking matrix dimensions. */
static int dataset_builder_append(dataset_builder_t *builder, const double *row_values, size_t row_count) {
...
/* Parse a text line into comma-separated doubles. */
static int parse_row(char *line, double *row_values, size_t *row_count) {
    char *token;
    size_t count = 0;
...
/* Return nonzero when the character is an ASCII whitespace. */
static int is_ascii_space(char ch) {
    return ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r' || ch == '\f' || ch == '\v';
}
