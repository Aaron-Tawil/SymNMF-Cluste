...
# Resolve project paths relative to this script and ensure we run from repo root
...
install_valgrind() {
    if ! command -v valgrind &> /dev/null; then
        echo -e "${RED}Valgrind not found. Please install it manually before running this tester.${NC}"
        exit 1
    else
...
install_brew() {
    echo -e "${BLUE}Homebrew detection skipped in this environment. Please ensure required tools are installed manually.${NC}"
}

# Install necessary packages
install_dependencies() {
    local missing=0
    for cmd in python3 make bc; do
    ...
    if [ $missing -ne 0 ]; then
        echo -e "${RED}Please install the missing dependencies manually before running the tester.${NC}"
        exit 1
    fi
...
# Paths to the project binaries and scripts
current_dir="${PROJECT_ROOT}"
prev_dir="${PROJECT_ROOT}/tests/legacy/Prev_final_100"
current_symnmf_path="${current_dir}/symnmf.py"
prev_symnmf_path="${prev_dir}/symnmf.py"

# Paths to the symnmf executables
...
python3 - "$file_name" "$num_points" "$dimensions" <<'PYGEN'
import random
import sys
file_name, num_points, dimensions = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
random.seed(1234)
with open(file_name, 'w', encoding='utf-8') as f:
    for _ in range(num_points):
        coords = [f"{random.random():.2f}" for _ in range(dimensions)]
        f.write(','.join(coords) + '\n')
PYGEN
}
